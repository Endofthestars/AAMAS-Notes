...
(function () {
  ...
  var T = {
    zh: {
      ...
      count: function (n, from, to, page, pages) {
        return pages > 1
          ? "找到 " + n + " 条结果（第 " + from + "-" + to + " 条 / 共 " + pages + " 页）"
          : "找到 " + n + " 条结果";
      },
      deep: function (q) {
  ...
  function buildDocs(raw) {
    ...
    for (var i = 0; i < raw.length; i++) {
      ...
      docs.push({
        loc: loc,
        title: title,
        venue: segs[0] === "notes" && /^\d{4}$/.test(segs[1]) ? "AAMAS" + segs[1] : segs[0],
        area: segs[0] === "notes" ? "Reviewed" : prettyArea(segs[1]),
        kw: kw,
        kwList: kwList,
        map: map,
  ...
  function deepHost() {
    return "endofthestars.github.io/AAMAS-Notes";
  }
  ...
  function ensurePager(list) {
    ...
    if (!pager) {
      ...
      pager.setAttribute("aria-label", LANG === "zh" ? "搜索结果分页" : "Search result pages");
